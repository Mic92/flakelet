...
A service flake exports one function under `flakelets.<attr>`. It has no
flake inputs. Everything it needs is injected by the host.
...
sockets."worker@" = {
  wantedBy = [ "sockets.target" ];
  instances = map toString (lib.range 1 options.workers);
