...
| `flakelet-<name>.service`     | oneshot `flakelet update --offline-fallback <name>`; restarts when the entry changes; after `flakelet-reconcile`, before `flakelet.target`; `Nice=10`, `IOSchedulingClass=idle`, `MemoryHigh=75%` |
