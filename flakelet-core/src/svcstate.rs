...
pub fn blockers(
    ...
    let mut out = Vec::new();
    if !state.opaque.is_empty() && state.folders.is_empty() {
        out.push(format!(
            "raw units {} cannot be inspected. Declare exports.state.extraFolders",
            state.opaque.join(", ")
        ));
    }
...
mod tests {
    ...
    #[test]
    fn opaque_units_and_providers_block_export() {
        let dir = tempfile::tempdir().unwrap();
        let state = StateInfo {
            opaque: vec!["web-raw.service".into()],
            ..Default::default()
        };
        ...
        assert_eq!(b.len(), 3, "{b:?}");
        assert!(b[0].contains("web-raw.service"));
        assert!(b[1].contains("postgres/v1 cannot dump"));
        assert!(b[2].contains("requires.s3"));
